...
class AdminControllerBuild extends Build
{
    ...
    public function run(): bool
    {
        ...
        foreach ($this->data['data'] as $value) {
            if ($value['form_type'] !== 'none') {
                ...
                switch ($value['relation_type']) {
                    ...
                    case 1:// 外键一对一
                    case 2:// 外键一对多
                        if ($value['field_name'] === 'parent_id' && $this->data['tree_show'] === 1) {
                            $relation_3 .= '\'parent_list\' => $this->getSelectList($model, $data->parent_id),';
                        } else {
            ...
            if ($value['index_search'] === 'select') {
                if ($value['relation_type'] === 1 || $value['relation_type'] === 2) {
                    ...
                    if($value['field_name']==='parent_id'){
                        $select_class_name = '$model';
                    }else{
                        $table_name        = $this->getSelectFieldFormat($value['field_name']);
        ...
        if ($relation_3 !== '') {
            ...
            $code_2     = str_replace(array('[RELATION_LIST]', ', $data->parent_id'), array($relation_3, ''), $code_2);
            $relation_2 = $code_2;
        }
    ...
    protected function createAction(): void
    {
        foreach ($this->actionList as $action) {
            ...
            if (in_array($action, $this->data['admin_controller']['action'], true)) {
                if($this->data['tree_show']===1 && ($action==='index' || $action==='del')){
                    $code = file_get_contents($this->template['action_' . $action.'_tree']);
                }else{
