...
class AdminViewBuild extends Build
{
    ...
    public function __construct(array $data, array $config)
    {
        ...
        if($this->data['tree_show']===1){
            $this->indexCode = file_get_contents($this->template['index_tree']);
        }else{
                ...
                if ($value['form_type'] === 'switch') {
                    ...
                    if ($value['relation_type'] === 1 || $value['relation_type'] === 2) {
                        if($value['field_name']==='parent_id'){
                            $value['option_data'] = '{$parent_list|raw}';
                        }else{
        ...
        foreach ($this->data['data'] as $value) {
            ...
            if ($value['is_list']) {
                ...
                if (in_array($value['form_type'], $file_fields, true)) {
                    ...
                    if ($value['relation_type'] === 1 || $value['relation_type'] === 2) {
                        // 关联字段显示
                        $temp_name = $this->getSelectFieldFormat($value['field_name']);
                        if ($value['field_name'] === 'parent_id') {
                            $temp_name = 'father';
                        }
                        $field_name = $temp_name . '.' . $value['relation_show'] . '|default=' . "''";
